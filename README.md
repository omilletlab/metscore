...
The base installation supports the Python API and CSV input. Excel support is kept optional to avoid installing additional dependencies when it is not needed.
...
MetSCORE can also be used directly from the terminal.
...
If no output path is provided, MetSCORE writes the result to the **current working directory** using the input filename with `_metscore` appended.
...
```bash
metscore samples.csv -o results.csv
```
...
Excel support requires the optional dependency:
